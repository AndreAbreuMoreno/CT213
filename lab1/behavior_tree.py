...
class RoombaBehaviorTree(BehaviorTree):
    ...
    def __init__(self):
        ...
        sequence_node1 = SequenceNode("node1")
        sequence_node1.add_child(MoveForwardNode())
        sequence_node1.add_child(MoveInSpiralNode())

        sequence_node2 = SequenceNode("node2")
        sequence_node2.add_child(GoBackNode())
        sequence_node2.add_child(RotateNode())

        self.root = SelectorNode ("root")
        self.root.add_child(sequence_node1)
        self.root.add_child(sequence_node2)
...
class RotateNode(LeafNode):
    ...
    def enter(self, agent):
        ...
        self.limite = self.angle / ANGULAR_SPEED

    def execute(self, agent):
        # Todo: add execution logic
        if self.time > self.limite:
            return ExecutionStatus.SUCCESS
        else:
            self.time += SAMPLE_TIME
            return ExecutionStatus.RUNNING
